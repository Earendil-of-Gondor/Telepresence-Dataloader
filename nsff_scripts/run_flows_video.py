...
def motion_segmentation(basedir, threshold):
    ...
    for i in range(0, len(glob.glob(basedir + '/images_*x*/*'))): #len(perm) - 1):
        cv2.imwrite(os.path.join(save_mask_dir, '{:05d}.png'.format(i)), np.zeros(shape_0))
...
def run_optical_flows(args):
    ...
    with torch.no_grad():
        ...
        for i in range(images.shape[0]-1):
            ...
            if VIZ:
                viz_flow_dir = os.path.join(basedir, './viz_flow')
                if not os.path.exists(viz_flow_dir):
                    os.makedirs(viz_flow_dir)

                viz_warp_imgs_dir = os.path.join(basedir, 'viz_warp_imgs')
                if not os.path.exists(viz_warp_imgs_dir):
                    os.makedirs(viz_warp_imgs_dir)
                ...
                plt.savefig(os.path.join(viz_flow_dir, '%02d.jpg'%i))
                plt.close()

                warped_im2 = warp_flow(img2, fwd_flow)
                warped_im0 = warp_flow(img1, bwd_flow)
  
                cv2.imwrite(os.path.join(viz_warp_imgs_dir, 'im_%05d.jpg'%(i)), img1[..., ::-1])
                cv2.imwrite(os.path.join(viz_warp_imgs_dir, 'im_%05d_fwd.jpg'%(i)), warped_im2[..., ::-1])
                cv2.imwrite(os.path.join(viz_warp_imgs_dir, 'im_%05d_bwd.jpg'%(i + 1)), warped_im0[..., ::-1])
